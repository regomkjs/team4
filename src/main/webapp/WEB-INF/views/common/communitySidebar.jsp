...
<div>
	<div class="side-tab">
		<ul class="side-tab-navi" style="padding: 0">
	...
	<c:if test="${user.me_mr_num < 2 }">
		<div class="dropup mb-3" style="width: 100%; height: 35px">
		    <button type="button" class="btn btn-outline-secondary dropdown-toggle" data-bs-toggle="dropdown" style="width: 100%;">
		      관리자 메뉴
		    </button>
				...
				<div class="container col-12">
					<div class="d-flex">
						...
						<ul class="nav flex-column nav-pills text-center" role="tablist" style="padding: 0">
							...
							<li class="nav-item">
								<a class="nav-link member-tab" data-bs-toggle="tab" href="#prison" data-type="prisoner">수감소</a>
							</li>
							<c:if test="${user.me_mr_num == 0}">
								<li class="nav-item">
									<a class="nav-link member-tab" data-bs-toggle="tab" href="#master" data-type="admin">운영진</a>
								</li>
							</c:if>
						</ul>
...
<div class="modal fade" id="reportDetailModal">
	<div class="modal-dialog modal-lg modal-dialog-scrollable">
		<div class="modal-content">
			...
			<!-- Modal footer -->
			<div class="modal-footer">
				<button type="button" class="btn btn-success me-auto complete-report-btn">처리완료</button>
				<button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
			</div>
		</div>
	</div>
					...
					`
							...
							<td>
								<span class="detail-link" data-id="\${report.rp_me_id}" data-num="\${report.rp_num}" data-writer="\${report.rp_writer_nick}" data-type="\${report.rp_type}" data-reporter="\${report.rp_me_nick}" data-note="\${report.rp_note}" data-post="\${report.rp_post.po_num}">
									<a class="reportDetailModal" href="#" data-bs-toggle="modal" data-bs-target="#reportDetailModal" ><button class="btn btn-sm btn-secondary" >상세보기</button></a>
					`
					if(report.rp_comment == null){
						str +=
						`
...
function setTable(where){
	...
	`
		<table class="table table-hover container text-center">
			<thead>
				<tr>
					<th class="order-btn order-right" data-order="right">권한<i class="fa-solid fa-caret-up order-icon ms-1"></i></th>
...
function addCaret(text, me_cri) {
	...
	let caret = "";
	if(me_cri.role == "asc"){
		caret += text + `<i class="fa-solid fa-caret-up order-icon ms-1"></i>` 
	}
	else{
		caret += text + `<i class="fa-solid fa-caret-down order-icon ms-1"></i>` 
	}
